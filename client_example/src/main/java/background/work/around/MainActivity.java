...
public final class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (lang.equals("ru")) {
            tv.setText("Привет! Это приложение пример клиента BackgroundWorkAround. Этот экран покажет вам приоритет процессов если это debug сборка. Это приложение может играть звук (если Android это позволяет)");
        } else {
            tv.setText("Hi! This app is example of client of BackgroundWorkAround. This screen shows you process priority if this is debug build. This app may play sound when started (if Android allows play it)");
        }
        ...
        if (lang.equals("ru")) {
        runBtn.setText("Эта кнопка чтобы дать uri permissions от <provider> если ещё не даны. Вероятно уже даны при первом запуске.");
        } else {
        runBtn.setText("This button to grant uri permissions from <provider> (if not granted). Probably granted on first launch.");
        }
